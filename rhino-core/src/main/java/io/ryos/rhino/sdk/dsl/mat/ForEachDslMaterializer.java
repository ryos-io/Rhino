...
public class ForEachDslMaterializer<S, R extends Iterable<S>> implements DslMaterializer<ForEachDsl<S, R>> {

  private static final Logger LOG = LoggerFactory.getLogger(ForEachDslMaterializer.class);

  @Override
  public Mono<UserSession> materialize(final ForEachDsl<S, R> forEachDsl, final UserSession session) {
    ...
    return Flux.fromIterable(iterable)
        ...
        .map(childDsl -> populateToChildren(forEachDsl, childDsl))
